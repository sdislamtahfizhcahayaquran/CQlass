(function(){
  function role(){try{return String((window.currentUser&&window.currentUser.role)||JSON.parse(localStorage.getItem('cqlass_session_user')||'{}').role||'').toLowerCase()}catch(e){return''}}
  ...
  document.addEventListener('DOMContentLoaded',function(){setTimeout(add,500)});setTimeout(add,1200);
})();