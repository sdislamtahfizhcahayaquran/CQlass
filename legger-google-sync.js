...
(function(){
  ...
  async function syncGroup(group){const u=getUser();try{const res=await callApi('saveLeggerNilai',{kelas:group.kelas,tahunAjaran:group.tahunAjaran,semester:group.semester,mapel:group.mapel,dicatatOleh:String(u?.nama||u?.name||'CQlass'),username:String(u?.username||'system_sync'),changes:Array.isArray(group.changes)?group.changes:[]});if(!res?.success)throw new Error(res?.error||'Legger belum berhasil diperbarui.');await queueReq('ack',{items:group.items||[]});return true}catch(err){try{await queueReq('fail',{items:group.items||[],error:String(err?.message||err||'sync_failed')})}catch(_){}return false}}
  async function drain(){if(draining||!navigator.onLine||typeof callApi!=='function'||!getToken())return;draining=true;try{for(let batch=0;batch<3;batch++){const claimed=await queueReq('claim',{limit:80});const groups=Array.isArray(claimed.groups)?claimed.groups:[];if(!groups.length)break;for(const group of groups)await syncGroup(group);if(Number(claimed.count||0)<80)break}}catch(_){}finally{draining=false}}
  function installAcademicHook(){if(typeof academicGridSave!=='function'||academicGridSave.__leggerGoogleHook)return false;const original=academicGridSave;const wrapped=async function(){const before=getState();const hadChanges=Boolean(before?.dirty?.size);await original.apply(this,arguments);const after=getState();if(hadChanges&&after?.dirty?.size===0)setTimeout(()=>void drain(),0)};wrapped.__leggerGoogleHook=true;window.academicGridSave=wrapped;return true}
  ...
  function start(){let tries=0;const hookTimer=setInterval(()=>{tries++;if(installAcademicHook()||tries>60)clearInterval(hookTimer)},500);enhanceMonitoringUI();const content=document.getElementById('content');if(content)new MutationObserver(enhanceMonitoringUI).observe(content,{childList:true,subtree:true});window.addEventListener('online',()=>void drain());setInterval(()=>void drain(),20000);setTimeout(()=>void drain(),1200)}
