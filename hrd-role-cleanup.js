...
function drawPromoRows(){
 ...
 body.innerHTML=rows.map(x=>{const last=x.items?.[0]||null;return `<tr><td><b>${esc(x.name)}</b></td><td>${esc((x.roles||[]).join(', ')||x.position||'Guru')}</td><td>${last?'<b>'+esc(promoWhen(last.created_at))+'</b>':'<span class="cq-hrd-chip bad">Belum upload</span>'}</td><td>${x.count||0}</td><td>${(x.items||[]).map((i,n)=>`<div class="cq-hrd-promo-evidence"><button class="cq-hrd-chip ok cq-hrd-promo-view" onclick="openHrdPromoPhoto('${esc(x.id||'')}','${n}')">${esc(promoDate(i.activity_date))} · Lihat Bukti</button></div>`).join('')||'—'}</td></tr>`}).join('')||'<tr><td colspan="5" class="cq-hrd-empty">Tidak ada data yang sesuai filter.</td></tr>';
}
window.filterHrdPromo=drawPromoRows;
window.openHrdPromoPhoto=(personId,index)=>{const row=(window.__cqHrdPromoRows||[]).find(x=>String(x.id||'')===String(personId));const i=row?.items?.[Number(index)];if(!i)return;const old=document.getElementById('cq-hrd-promo-modal');if(old)old.remove();const m=document.createElement('div');m.id='cq-hrd-promo-modal';m.className='cq-hrd-modal';m.onclick=e=>{if(e.target===m)m.remove()};m.innerHTML=`<div class="cq-hrd-modal-card"><div class="cq-hrd-modal-head"><b>Bukti Promo Socmed — ${esc(row.name||'')}</b><button class="cq-hrd-modal-close" onclick="document.getElementById('cq-hrd-promo-modal')?.remove()">Tutup</button></div>${i.photo_url?`<img class="cq-hrd-modal-img" src="${esc(i.photo_url)}" alt="Bukti Promo Socmed">`:'<div class="cq-hrd-empty">Foto tidak tersedia.</div>'}<div class="cq-hrd-modal-meta"><b>Tanggal kegiatan:</b> ${esc(promoDate(i.activity_date))}<br><b>Waktu upload:</b> ${esc(promoWhen(i.created_at))}${i.description?`<br><b>Keterangan:</b> ${esc(i.description)}`:''}</div></div>`;document.body.appendChild(m)};
