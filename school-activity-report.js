...
(function(){
  ...
  function renderBody(){
    ...
    const rows=students.map(st=>`<tr data-student="${st.id}"><td><div class="sar-name">${esc(st.name)}</div><div class="sar-sub">${esc(st.nis||'')}</div></td>${acts.map(ac=>{const can=editable.has(ac.code),checked=Boolean(st.marks?.[ac.code]);return`<td><input type="checkbox" class="sar-check ${can?'editable':''}" data-student="${st.id}" data-code="${esc(ac.code)}" ${checked?'checked':''} ${S.editing&&can?'':'disabled'} onchange="sarLive()" title="${can?'':'Tidak dapat diedit oleh akun ini'}"></td>`}).join('')}<td><span class="sar-grade" title="${st.percentage==null?'Belum ada kegiatan':`${st.checked}/${st.total} · ${st.percentage}%`}">${esc(st.grade||'—')}</span></td></tr>`).join('');
    root.innerHTML=`<div class="sar-card"><div class="sar-table-wrap"><table class="sar-table"><thead><tr><th>Nama Siswa</th>${heads}<th>Predikat</th></tr></thead><tbody>${rows||'<tr><td colspan="9" class="sar-empty">Tidak ada siswa aktif.</td></tr>'}</tbody></table></div></div>`;
  ...
  window.sarLive=function(){
    ...
    root.querySelectorAll('tbody tr[data-student]').forEach(tr=>{
      ...
      const total=used.length,pct=total?Math.round(checked/total*100):null,g=tr.querySelector('.sar-grade');
      if(g){g.textContent=grade(pct);g.title=pct==null?'Belum ada kegiatan':`${checked}/${total} · ${pct}%`;}
    });
  };
