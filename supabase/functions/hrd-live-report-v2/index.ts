...
function weekdaysInRange(start:string,end:string){const out=new Set<number>(),a=new Date(`${start}T00:00:00Z`),z=new Date(`${end}T00:00:00Z`);if(!Number.isFinite(a.getTime())||!Number.isFinite(z.getTime())||a>z)return out;for(let d=new Date(a),i=0;d<=z&&i<7;i++,d.setUTCDate(d.getUTCDate()+1)){const js=d.getUTCDay();out.add(js===0?7:js)}return out}

const DISPLAY_NOTES:Record<string,string>={
  rpp:"RPP sesuai target pembelajaran yang menjadi tanggung jawab guru pada periode terpilih.",
  timesheet:"Timesheet mencakup catatan mengajar/JP dan aktivitas kerja yang tersimpan di CQlass.",
  academic:`Nilai TP dibaca langsung dari input nilai. Untuk ringkasan rapor PTS, satu penugasan dinilai tuntas jika minimal ${MIN_REPORT_TP} TP sudah memiliki nilai siswa.`,
  tahfizh:"Laporan Tahfizh pada kelas/halaqah yang menjadi tanggung jawab guru.",
  bilingual:"Progres Bilingual pada kelas yang menjadi tanggung jawab guru.",
  pjbl:"Kegiatan PBL/Market Day yang sudah dicatat untuk kelas yang menjadi tanggung jawab wali kelas.",
  promotion:"Bukti promosi sekolah yang sudah tercatat.",
  principal_work:"Jurnal dan rencana kerja Kepala Sekolah yang sudah tercatat.",
  attendance:"Kehadiran kegiatan ditampilkan sebagai informasi operasional.",
  badal:"Riwayat menggantikan atau digantikan pada periode terpilih.",
  student_affairs_reporting:"Status laporan Reward & Kedisiplinan untuk wali kelas.",
  workload:"Ringkasan beban kerja berdasarkan data yang sudah tersimpan di CQlass."
};
function cleanCategoryNotes(t:any){for(const c of [...(t.categories||[]),...(t.context_categories||[])])if(DISPLAY_NOTES[c?.key])c.note=DISPLAY_NOTES[c.key]}

async function enrichPhotos(teachers:any[]){
  try{
    const teacherIds=uniq(teachers.map(t=>T(t.teacher_id)));if(!teacherIds.length)return;
    const {data:accounts,error:ae}=await sb.from("user_accounts").select("id,teacher_id").in("teacher_id",teacherIds);if(ae)throw ae;
    const accountIds=uniq((accounts||[]).map((a:any)=>T(a.id)));if(!accountIds.length)return;
    const {data:photos,error:pe}=await sb.from("user_profile_photos").select("user_account_id,storage_path,updated_at").in("user_account_id",accountIds);if(pe)throw pe;
    const paths=uniq((photos||[]).map((p:any)=>T(p.storage_path))),signedByPath=new Map<string,string>();
    if(paths.length){const {data:signed,error:se}=await sb.storage.from("profile-photos").createSignedUrls(paths,21600);if(!se)for(const x of signed||[])if(x?.path&&x?.signedUrl)signedByPath.set(T(x.path),T(x.signedUrl))}
    const accountByTeacher=new Map<string,string>((accounts||[]).map((a:any)=>[T(a.teacher_id),T(a.id)])),photoByAccount=new Map<string,any>((photos||[]).map((p:any)=>[T(p.user_account_id),p]));
    for(const t of teachers){const aid=accountByTeacher.get(T(t.teacher_id)),p=aid?photoByAccount.get(aid):null;t.profile_photo_url=p?signedByPath.get(T(p.storage_path))||null:null;t.profile_photo_updated_at=p?.updated_at||null}
...
async function enrichAcademicTP(teachers:any[]){
  try{
    ...
    const cm=new Map<string,any>(classes.map((x:any)=>[T(x.id),x])),sm=new Map<string,any>(subjects.map((x:any)=>[T(x.id),x])),objByScope=new Map<string,any[]>();
    for(const o of objectives){const key=`${T(o.subject_id)}|${Number(o.grade_level)||0}`;if(!objByScope.has(key))objByScope.set(key,[]);objByScope.get(key)!.push(o)}
    ...
    const scoresByTeacher=new Map<string,any[]>();for(const s of scores){const id=T(s.teacher_id);if(!scoresByTeacher.has(id))scoresByTeacher.set(id,[]);scoresByTeacher.get(id)!.push(s)}

    for(const t of teachers){
      const tid=T(t.teacher_id),teacherAssignments=assignments.filter((a:any)=>T(a.teacher_id)===tid),teacherScores=scoresByTeacher.get(tid)||[],detail:any[]=[];
      for(const a of teacherAssignments){
        ...
        if(!scopeObjectives.length&&scopeScores.length){const ids=uniq(scopeScores.map((x:any)=>T(x.learning_objective_id)));scopeObjectives=objectives.filter((o:any)=>ids.includes(T(o.id))).sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0))}
        const tp=scopeObjectives.map((o:any,idx:number)=>{
          const rows=scopeScores.filter((x:any)=>T(x.learning_objective_id)===T(o.id)),students=uniq(rows.map((x:any)=>T(x.student_id))).length,vals=rows.map((x:any)=>Number(x.score)).filter((x:number)=>Number.isFinite(x)),avg=vals.length?Math.round((vals.reduce((z:number,v:number)=>z+v,0)/vals.length)*10)/10:null;
          return{id:o.id,code:`TP${idx+1}`,source_code:T(o.code)||null,description:T(o.description)||T(o.topic)||"Tujuan pembelajaran",topic:T(o.topic)||null,kktp:o.kktp??null,scored_students:students,score_rows:rows.length,average_score:avg,last_updated_at:lastOf(rows),status:students>0?"entered":"missing"};
        });
        const entered=tp.filter((x:any)=>x.status==="entered").length;
        detail.push({class_id:cid,class_name:T(cl?.name)||"Kelas",grade_level:grade,subject_id:sid,subject_name:T(sub?.name)||T(sub?.code)||"Mapel",tp,entered_tp:entered,total_tp:tp.length,report_minimum_tp:MIN_REPORT_TP,report_complete:entered>=MIN_REPORT_TP,students_scored:uniq(scopeScores.map((x:any)=>T(x.student_id))).length,score_rows:scopeScores.length,last_updated_at:lastOf(scopeScores)});
      }
      const sorted=detail.sort((a,b)=>a.class_name.localeCompare(b.class_name,"id")||a.subject_name.localeCompare(b.subject_name,"id"));
      const complete=sorted.filter(x=>x.report_complete).length,one=sorted.filter(x=>Number(x.entered_tp)===1).length,empty=sorted.filter(x=>Number(x.entered_tp)===0).length;
      t.academic_tp_detail=sorted;
      t.academic_tp_summary={assignments:sorted.length,assignments_complete:complete,assignments_one_tp:one,assignments_empty:empty,minimum_tp_per_assignment:MIN_REPORT_TP,entered_tp:sorted.reduce((z,x)=>z+Number(x.entered_tp||0),0),total_tp:sorted.reduce((z,x)=>z+Number(x.total_tp||0),0),students_scored:uniq(teacherScores.filter((x:any)=>T(x.learning_objective_id)).map((x:any)=>T(x.student_id))).length,score_rows:teacherScores.filter((x:any)=>T(x.learning_objective_id)).length};
    }
  }catch(e){console.error("hrd-live-report-v2 academic TP enrichment",e)}
}

function normalizeTeacher(t:any){
  const cats=Array.isArray(t.categories)?t.categories:[];
  const work=cats.find((c:any)=>c?.key==="work_activity"),timesheet=cats.find((c:any)=>c?.key==="timesheet");
  if(timesheet&&work){
    const merged=[...(timesheet.activity_items||[]),...(work.items||[])],seen=new Set<string>();
    timesheet.activity_items=merged.filter((x:any)=>{const k=T(x.id)||`${T(x.period_start)}|${T(x.title)}|${T(x.description)}`;if(seen.has(k))return false;seen.add(k);return true});
    timesheet.activity_item_count=timesheet.activity_items.length;
...
Deno.serve(async(req)=>{
  ...
  try{
    const body=await req.json().catch(()=>({})),upstream=await fetch(`${URL}/functions/v1/hrd-live-report`,{method:"POST",headers:{"Content-Type":"application/json","apikey":req.headers.get("apikey")||"","Authorization":req.headers.get("Authorization")||"","x-session-token":req.headers.get("x-session-token")||""},body:JSON.stringify(body)}),data=await upstream.json().catch(()=>({success:false,error:"invalid_upstream"}));
    if(!upstream.ok||data?.success===false)return J(data,upstream.status);if(T(body?.action).toLowerCase()!=="administration")return J(data,upstream.status);
    const start=T(body.start),end=T(body.end),periodDays=weekdaysInRange(start,end),{data:schedule,error}=await sb.from("uks_duty_schedule").select("teacher_id,weekday,shift_no,start_time,end_time,is_active").eq("is_active",true).order("weekday").order("shift_no");if(error)throw error;
