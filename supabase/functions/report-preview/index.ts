...
async function period(s:any,yearName:string,semesterNo:number){const {data:unit,error:ue}=await s.from("school_units").select("id").eq("code",SCHOOL_UNIT_CODE).single();if(ue||!unit)throw new Error("school_unit_not_found");let yq=s.from("academic_years").select("id,name").eq("school_unit_id",unit.id);if(yearName)yq=yq.eq("name",yearName);else yq=yq.eq("is_active",true);const {data:year,error:ye}=await yq.order("created_at",{ascending:false}).limit(1).maybeSingle();if(ye||!year)throw new Error("academic_year_not_found");let sem=semesterNo;if(!sem){const {data:sr,error:se}=await s.from("semesters").select("semester_no").eq("academic_year_id",year.id).eq("is_active",true).limit(1).maybeSingle();if(se)throw se;sem=Number(sr?.semester_no||0)}if(![1,2].includes(sem))throw new Error("active_semester_not_found");return{unit,year,semesterNo:sem}}
