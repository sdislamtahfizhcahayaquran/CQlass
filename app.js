...
function renderLegger(content){
  ...
  content.innerHTML = `
    ...
    <div class="page-sub">Input nilai cukup satu kali. Nilai yang sudah diimpor dari Legger Excel akan langsung muncul di sini.</div>
...
async function loadLeggerSetup(){
  ...
  try{
    ...
    const isAdminView = ['admin','kesiswaan','pimpinan'].includes(String(currentUser?.role || '').toLowerCase());
    const msg = (!isAdminView && /sheet tidak ditemukan|spreadsheet aktif|database mode|sheet tersedia/i.test(rawMsg))
      ? 'Data Legger belum siap. Silakan hubungi admin.'
      : rawMsg;
...
async function loadLeggerMapel(mapel){
  ...
  if(chip) chip.textContent = legerState.kkm === null ? 'KKM belum diatur' : `KKM ${legerState.kkm}`;
  ...
  try{
    ...
    const isAdminView = ['admin','kesiswaan','pimpinan'].includes(String(currentUser?.role || '').toLowerCase());
    const msg = (!isAdminView && /sheet tidak ditemukan|spreadsheet aktif|database mode|sheet tersedia/i.test(rawMsg))
      ? 'Data nilai belum siap. Silakan hubungi admin.'
      : rawMsg;
...
function renderLeggerTable(){
  ...
  if(!legerState.komponen.length){
    area.innerHTML = `<div class="empty-state"><div class="icon">—</div>MasterKomponenNilai untuk ${escapeHtml(legerState.mapel)} belum tersedia.</div>`;
    return;
  }
    ...
    <div class="card lg-table-card">
      <div class="lg-table-head">
        <div>
          ...
          <div class="lg-note">Rata-rata dan Nilai Akhir dihitung otomatis. Nilai disimpan hanya saat tombol Simpan Perubahan ditekan.</div>
        </div>
        <button class="btn btn-sm lg-save-btn" id="lg-save-btn" onclick="saveLeggerChanges()" disabled>Simpan Perubahan</button>
      </div>

      ${legerState.kkm === null ? `<div class="lg-warning">KKM ${escapeHtml(legerState.mapel)} belum ditemukan di MasterKKM. Nilai tetap bisa diinput, tetapi status ketuntasan belum dapat ditentukan.</div>` : ''}
...
async function saveLeggerChanges(){
  ...
  try{
    ...
    const isAdminView = ['admin','kesiswaan','pimpinan'].includes(String(currentUser?.role || '').toLowerCase());
    const msg = (!isAdminView && /sheet tidak ditemukan|spreadsheet aktif|database mode|sheet tersedia/i.test(rawMsg))
      ? 'Nilai belum dapat disimpan. Silakan hubungi admin.'
      : rawMsg;
    showToast(msg, true);
    updateLeggerSaveState();
  }finally{
