...
/* ==========================================================
   MODUL: BILINGUAL BULANAN
   ========================================================== */
let bilingualState = { kelas:null, tahunAjaran:'2026/2027', semester:1, bulan:'', data:null, dirty:false, loadToken:0 };
function renderBilingualBulanan(content){
  const isWalas=currentUser.role==='walas';
  bilingualState={kelas:isWalas?currentUser.kelas:null,tahunAjaran:'2026/2027',semester:1,bulan:cpDefaultBulan(1),data:null,dirty:false,loadToken:bilingualState.loadToken||0};
  content.innerHTML=`<div class="page-title">Bilingual</div><div class="page-sub">Catat kegiatan dan perkembangan bilingual kelas setiap bulan.</div>${!isWalas?`<div class="card"><div class="card-title">Pilih Kelas</div><div class="bi-admin-class-row"><input id="bi-kelas-admin" type="text" placeholder="Contoh: 3A Banat"><button class="btn btn-sm" onclick="bilingualLoadClassAdmin()">Tampilkan</button></div></div>`:''}<div id="bi-main"></div>`;
  if(isWalas&&bilingualState.kelas)loadBilingualBulanan();
}
function bilingualLoadClassAdmin(){const kelas=(document.getElementById('bi-kelas-admin')?.value||'').trim();if(!kelas){showToast('Isi nama kelas terlebih dahulu.',true);return;}bilingualState.kelas=kelas;loadBilingualBulanan();}
async function loadBilingualBulanan(){
  if(!bilingualState.kelas)return;const token=++bilingualState.loadToken,main=document.getElementById('bi-main');if(!main)return;
  main.innerHTML=`<div class="card bi-loading"><span class="spinner" style="border-top-color:var(--primary);border-color:rgba(10,110,110,.25)"></span>Memuat data bilingual...</div>`;
  try{const res=await callApi('getBilingualBulanan',{kelas:bilingualState.kelas,tahunAjaran:bilingualState.tahunAjaran,semester:bilingualState.semester,bulan:bilingualState.bulan});if(token!==bilingualState.loadToken||activeModule!=='bilingual')return;if(!res.success)throw new Error(res.error||'Data bilingual belum dapat dibaca.');bilingualState.tahunAjaran=res.tahunAjaran||bilingualState.tahunAjaran;bilingualState.semester=Number(res.semester)||bilingualState.semester;bilingualState.bulan=res.bulan||bilingualState.bulan;bilingualState.data=res.data||{};bilingualState.dirty=false;renderBilingualForm();}
  catch(err){if(token!==bilingualState.loadToken||activeModule!=='bilingual')return;main.innerHTML=`<div class="empty-state"><div class="icon">—</div>${escapeHtml(userFriendlyDataMessage(err.message,'Data bilingual belum tersedia. Silakan hubungi admin.'))}</div>`;}
}
function renderBilingualForm(){
  const main=document.getElementById('bi-main');if(!main)return;const bulanOptions=(CP_BULAN_SEMESTER[Number(bilingualState.semester)]||[]).map(b=>`<option value="${escapeHtml(b)}" ${b===bilingualState.bulan?'selected':''}>${escapeHtml(b)}</option>`).join('');const d=bilingualState.data||{};
  main.innerHTML=`<div class="card bi-toolbar-card"><div class="bi-filter-grid"><div class="bi-field"><label>Kelas</label><div class="bi-static">${escapeHtml(bilingualState.kelas||'-')}</div></div><div class="bi-field"><label>Tahun Ajaran</label><input id="bi-ta" value="${escapeHtml(bilingualState.tahunAjaran)}" onchange="bilingualPeriodChanged()"></div><div class="bi-field"><label>Semester</label><select id="bi-semester" onchange="bilingualSemesterChanged()"><option value="1" ${Number(bilingualState.semester)===1?'selected':''}>Semester 1</option><option value="2" ${Number(bilingualState.semester)===2?'selected':''}>Semester 2</option></select></div><div class="bi-field"><label>Bulan</label><select id="bi-bulan" onchange="bilingualBulanChanged(this.value)">${bulanOptions}</select></div></div><div class="bi-toolbar-meta"><span class="bi-save-state" id="bi-save-state">Tersimpan</span></div></div>
  <div class="card bi-form-card"><div class="bi-section-title">Bulan ${escapeHtml(bilingualState.bulan)}</div><div class="bi-field bi-field-full"><label>Tema / Kegiatan</label><textarea id="bi-tema" class="bi-textarea" placeholder="Contoh: Daily classroom expressions, greeting, asking permission..." oninput="bilingualChanged()">${escapeHtml(d.tema||'')}</textarea></div><div class="bi-field bi-field-full"><label>Kosakata / Ungkapan yang Dipelajari</label><textarea id="bi-kosakata" class="bi-textarea" placeholder="Tuliskan kosakata atau ungkapan yang digunakan selama bulan ini." oninput="bilingualChanged()">${escapeHtml(d.kosakata||'')}</textarea></div><div class="bi-field bi-field-full"><label>Capaian Kelas</label><textarea id="bi-capaian" class="bi-textarea" placeholder="Tuliskan perkembangan atau capaian siswa secara singkat." oninput="bilingualChanged()">${escapeHtml(d.capaian||'')}</textarea></div><div class="bi-field bi-field-full"><label>Catatan Tambahan <span class="bi-optional">(opsional)</span></label><textarea id="bi-catatan" class="bi-textarea bi-textarea-short" placeholder="Catatan singkat bila diperlukan." oninput="bilingualChanged()">${escapeHtml(d.catatan||'')}</textarea></div><div class="bi-actions"><button class="btn bi-save-btn" id="bi-save-btn" onclick="saveBilingualBulanan()" disabled>Simpan</button></div></div>`;bilingualUpdateSaveState();
}
function bilingualChanged(){bilingualState.dirty=true;bilingualUpdateSaveState();}
function bilingualUpdateSaveState(){const b=document.getElementById('bi-save-btn'),s=document.getElementById('bi-save-state');if(b)b.disabled=!bilingualState.dirty;if(s){s.textContent=bilingualState.dirty?'Belum disimpan':'Tersimpan';s.classList.toggle('dirty',bilingualState.dirty);}}
function bilingualPeriodChanged(){if(bilingualState.dirty&&!confirm('Ada perubahan yang belum disimpan. Tetap ganti tahun ajaran?'))return;bilingualState.tahunAjaran=(document.getElementById('bi-ta')?.value||'').trim()||'2026/2027';loadBilingualBulanan();}
function bilingualSemesterChanged(){if(bilingualState.dirty&&!confirm('Ada perubahan yang belum disimpan. Tetap ganti semester?')){const e=document.getElementById('bi-semester');if(e)e.value=String(bilingualState.semester);return;}bilingualState.semester=Number(document.getElementById('bi-semester')?.value)||1;bilingualState.bulan=cpDefaultBulan(bilingualState.semester);loadBilingualBulanan();}
function bilingualBulanChanged(bulan){if(bilingualState.dirty&&!confirm('Ada perubahan yang belum disimpan. Tetap ganti bulan?')){const e=document.getElementById('bi-bulan');if(e)e.value=bilingualState.bulan;return;}bilingualState.bulan=bulan;loadBilingualBulanan();}
async function saveBilingualBulanan(){
  const tema=(document.getElementById('bi-tema')?.value||'').trim(),kosakata=(document.getElementById('bi-kosakata')?.value||'').trim(),capaian=(document.getElementById('bi-capaian')?.value||'').trim(),catatan=(document.getElementById('bi-catatan')?.value||'').trim();if(!tema&&!kosakata&&!capaian&&!catatan){showToast('Isi minimal satu bagian sebelum menyimpan.',true);return;}const btn=document.getElementById('bi-save-btn');btn.disabled=true;btn.innerHTML='<span class="spinner"></span>Menyimpan...';
  try{const res=await callApi('saveBilingualBulanan',{kelas:bilingualState.kelas,tahunAjaran:bilingualState.tahunAjaran,semester:bilingualState.semester,bulan:bilingualState.bulan,tema,kosakata,capaian,catatan,dicatatOleh:currentUser.nama,username:currentUser.username});if(!res.success)throw new Error(res.error||'Data bilingual belum dapat disimpan.');bilingualState.data={tema,kosakata,capaian,catatan};bilingualState.dirty=false;bilingualUpdateSaveState();showToast('Data bilingual berhasil disimpan.');}
  catch(err){showToast(userFriendlyDataMessage(err.message,'Data bilingual belum dapat disimpan. Silakan hubungi admin.'),true);}finally{const b=document.getElementById('bi-save-btn');if(b){b.textContent='Simpan';b.disabled=!bilingualState.dirty;}}
}
