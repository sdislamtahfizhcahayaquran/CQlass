...
let runtimeConfig = {
  tahunAjaran: '',
  semester: runtimeConfig.semester || 1,
  periode: '',
  notifIntervalMs: 300000
};
...
async function callApi(action, params={}){
  ...
  try{
    ...
    if(networkErr?.name === 'AbortError'){
      throw new Error(action === 'login'
        ? 'Terjadi kendala. Silakan hubungi admin.'
        : 'Terjadi kendala. Silakan hubungi admin.');
    }
    throw new Error('Terjadi kendala. Silakan hubungi admin.');
  } finally {
  ...
  if(!data.success && action !== 'login' && action !== 'gantiPassword' && action !== 'gantiUsername'){
    data.error = 'Terjadi kendala. Silakan hubungi admin.';
...
document.getElementById('login-form').addEventListener('submit', async (e) => {
  ...
  try{
    ...
    if(res.success){
      ...
      runtimeConfig = { ...runtimeConfig, ...(res.config || {}) };
      sessionStorage.setItem('kesiswaan_user', JSON.stringify(currentUser));
      sessionStorage.setItem('cqlass_runtime_config', JSON.stringify(runtimeConfig));
      enterApp();
    } else {
...
function logout(){
  ...
  sessionStorage.removeItem('cqlass_runtime_config');
  currentUser = null;
...
window.addEventListener('load', () => {
  ...
  const savedCfg = sessionStorage.getItem('cqlass_runtime_config');
  if(savedCfg){ try{ runtimeConfig = { ...runtimeConfig, ...JSON.parse(savedCfg) }; }catch(e){} }
  if(saved){
...
const MODULE_GROUPS = [
  {
    id: 'akademik', label: 'Akademik', roles: ['walas','kesiswaan','pimpinan','admin'],
    items: [
      { id: 'leger',      label: 'Legger Nilai', roles: ['walas','kesiswaan','pimpinan','admin'], built: true,  render: renderLegger },
      { id: 'cp',         label: 'Capaian Pembelajaran', roles: ['walas','kesiswaan','pimpinan','admin'], built: true, render: renderCPBulanan },
      { id: 'bilingual',  label: 'Bilingual', roles: ['walas','kesiswaan','pimpinan','admin'], built: true, render: renderVocabularyBulanan },
      { id: 'pjbl',       label: 'PjBL',        roles: ['walas','kesiswaan','pimpinan','admin'], built: true,  render: renderPjBL },
      { id: 'rapor',    label: 'Cetak Rapor', roles: ['walas','kesiswaan','pimpinan','admin'], built: true,  render: renderCetakRapor }
    ]
  },
  {
    id: 'kesiswaan', label: 'Kesiswaan', roles: ['walas','kesiswaan','pimpinan','admin'],
    items: [
      { id: 'absensi',      label: 'Absensi (Morning Talk)', roles: ['walas','kesiswaan','pimpinan','admin'], built: true, render: renderAbsensi },
      { id: 'kedisiplinan', label: 'Kedisiplinan',           roles: ['walas','kesiswaan','pimpinan','admin'], built: true, render: renderKedisiplinan },
      { id: 'reward',       label: 'Reward Siswa',           roles: ['walas','kesiswaan','pimpinan','admin'], built: true, render: renderReward },
      { id: 'masalah',      label: 'Masalah Siswa',          roles: ['walas','kesiswaan','pimpinan','admin'], built: true, render: renderMasalahSiswa }
    ]
  },
  {
    id: 'info', label: 'Info', roles: ['walas','kesiswaan','pimpinan','admin'],
    items: [
      { id: 'ekskul', label: 'Ekskul', roles: ['walas','kesiswaan','pimpinan','admin'], built: false },
      { id: 'ttd-admin', label: 'Tanda Tangan', roles: ['kesiswaan','pimpinan','admin'], built: true, render: renderTTDAdmin }
    ]
  },
  {
    id: 'laporan', label: 'Laporan', roles: ['walas','kesiswaan','pimpinan','admin'],
    items: [
      { id: 'laporan-guru',   label: 'Laporan Guru Bulanan', roles: ['walas','kesiswaan','pimpinan','admin'], built: true,  render: renderLaporanGuru },
...
const DASHBOARD_MODULE = { id: 'dashboard', label: 'Dashboard', roles: ['walas','kesiswaan','pimpinan','admin'], built: true, render: renderDashboard };
  ...
  if(currentUser.role === 'walas'){
    ...
    if(!bellIntervalId){
      bellIntervalId = setInterval(() => {
        if(currentUser?.role === 'walas') refreshBellNotif();
      }, Number(runtimeConfig.notifIntervalMs || 300000));
    }
  }
  ...
  area.innerHTML = `
    <div class="card">
      ...
      <div class="ms-actions">
        <button class="btn" id="ms-ai-btn" onclick="generateSaranMasalahAI()">Buat Saran Singkat</button>
...
async function generateSaranMasalahAI(){
  ...
  btn.disabled = true;
  btn.innerHTML = '<span class="spinner"></span>Menyusun saran...';
  result.innerHTML = `<div class="card"><span class="spinner" style="border-top-color:var(--primary);border-color:rgba(10,110,110,0.25)"></span>Menyusun rekomendasi singkat...</div>`;
...
function renderAbsensiSudahDiisi(cek){
  // Walas tetap dapat mengedit absensi Morning Talk yang sudah tersimpan.
  absensiState.siswa = Array.isArray(cek.data) ? cek.data.map(x => ({
    nis:String(x.nis || ''), nama:String(x.nama || '')
  })) : [];
  absensiState.status = {};
  (cek.data || []).forEach(x => absensiState.status[String(x.nis)] = x.status || 'Hadir');
  absensiState.tema = cek.temaMT || '';
  ...
  body.innerHTML = `
    <div class="card">
      <div class="card-title">Absensi hari ini sudah tersimpan</div>
      <div style="font-size:12.5px;color:var(--muted);line-height:1.6;margin-bottom:14px;">
        Data masih dapat diperbaiki oleh wali kelas. Tekan <strong>Edit Absensi</strong> untuk mengubah status siswa atau tema Morning Talk.
      </div>
      <div class="rekap-grid">
        <div class="rekap-box"><div class="num" style="color:var(--success)">${cek.rekap.Hadir||0}</div><div class="lbl">Hadir</div></div>
        <div class="rekap-box"><div class="num" style="color:var(--warn)">${cek.rekap.Sakit||0}</div><div class="lbl">Sakit</div></div>
        <div class="rekap-box"><div class="num" style="color:#6A8FA6">${cek.rekap.Izin||0}</div><div class="lbl">Izin</div></div>
        <div class="rekap-box"><div class="num" style="color:var(--danger)">${cek.rekap.Alfa||0}</div><div class="lbl">Alfa</div></div>
      </div>
      <button class="btn btn-outline" onclick="renderAbsensiForm()">Edit Absensi</button>
    </div>
  `;
}
...
function renderAbsensiForm(){
  ...
  body.innerHTML = `
    ...
    <button class="btn" id="submit-absensi-btn" onclick="submitAbsensiForm()">Simpan / Perbarui Absensi</button>
  `;
...
async function submitAbsensiForm(){
  ...
  try{
    ...
    btn.disabled = false;
    btn.textContent = 'Simpan / Perbarui Absensi';
  }
}
...
let legerState = {
  kelas: null,
  tahunAjaran: runtimeConfig.tahunAjaran || '',
  semester: runtimeConfig.semester || 1,
  jenjang: null,
  mapelList: [],
  mapel: '',
...
function legerPeriodChanged(){
  ...
  legerState.tahunAjaran = (document.getElementById('lg-ta')?.value || '').trim() || runtimeConfig.tahunAjaran || '';
...
function cpPeriodChanged(){
  ...
  cpState.tahunAjaran = (document.getElementById('cp-ta')?.value || '').trim() || runtimeConfig.tahunAjaran || '';
...
function vocabPeriodChanged(){
  ...
  vocabState.tahunAjaran=(document.getElementById('vc-ta')?.value||'').trim()||runtimeConfig.tahunAjaran||'';
  loadVocabularyBulanan();
}
...
function renderKedisiplinanShell(){
  ...
  body.innerHTML = `
    ...
    <div class="kd-tabs">
      ...
      <button class="kd-tab ${kdState.tab==='telat'?'active':''}" onclick="switchKdTab('telat')">+ Catat Terlambat</button>
...
function renderKdTabContent(){
  ...
  else if(kdState.tab === 'telat') renderKdInputKeterlambatan();
  else renderKdInput();
}
...
/* ==========================================================
   MODUL: CETAK RAPOR PTS — PDF PER SISWA + ZIP PER KELAS
   ========================================================== */
let raporState = { kelas:null, periode:runtimeConfig.periode || '', siswa:[], loading:false };

function renderCetakRapor(content){
  const isWalas = currentUser.role === 'walas';
  raporState = { kelas:isWalas ? currentUser.kelas : null, periode:runtimeConfig.periode || '', siswa:[], loading:false };
  content.innerHTML = `
    <div class="page-title">Cetak Rapor PTS</div>
    <div class="page-sub">Unduh PDF per siswa atau seluruh siswa dalam satu kelas sebagai ZIP.</div>

    ${!isWalas ? `<div class="card rp-filter-card">
      <div class="card-title">Pilih Kelas</div>
      <div class="rp-filter-row">
        <input type="text" id="rp-kelas-input" placeholder="Contoh: 3A Banat">
        <button class="btn btn-sm" onclick="loadRaporPTSStudents()">Tampilkan</button>
      </div>
    </div>` : ''}

    <div class="card rp-period-card">
      <div class="rp-period-grid">
        <div class="rp-field"><label>Semester</label><select id="rp-semester"><option value="1">Semester 1</option><option value="2">Semester 2</option></select></div>
        <div class="rp-field"><label>Periode</label><select id="rp-jenis"><option value="PTS">PTS / Tengah Semester</option></select></div>
      </div>
    </div>
    <div id="rp-main"></div>`;
  document.getElementById('rp-semester')?.addEventListener('change', () => { raporState.periode = periodeKodeRapor(); });
  if(isWalas) loadRaporPTSStudents();
}

function periodeKodeRapor(){
  const semester = document.getElementById('rp-semester')?.value || '1';
  return `S${semester}_PTS`;
}

async function loadRaporPTSStudents(){
  if(currentUser.role !== 'walas') raporState.kelas = (document.getElementById('rp-kelas-input')?.value || '').trim();
  raporState.periode = periodeKodeRapor();
  const main = document.getElementById('rp-main');
  if(!raporState.kelas){ showToast('Pilih kelas terlebih dahulu.', true); return; }
  main.innerHTML = `<div class="card"><span class="spinner" style="border-top-color:var(--primary);border-color:rgba(10,110,110,.25)"></span>Memuat daftar siswa...</div>`;
  try{
    const res = await callApi('getRaporPTSStudents', { kelas:raporState.kelas, periode:raporState.periode });
    if(!res.success) throw new Error('Terjadi kendala. Silakan hubungi admin.');
    raporState.siswa = res.data || [];
    renderRaporPTSStudentList();
  }catch(err){
    main.innerHTML = `<div class="empty-state"><div class="icon">—</div>Terjadi kendala. Silakan hubungi admin.</div>`;
  }
}

function renderRaporPTSStudentList(){
  const main = document.getElementById('rp-main');
  if(!raporState.siswa.length){
    main.innerHTML = `<div class="empty-state"><div class="icon">—</div>Belum ada siswa pada kelas ini.</div>`;
    return;
  }
  main.innerHTML = `
    <div class="card rp-download-card">
      <div class="rp-download-head">
        <div><div class="card-title" style="margin-bottom:3px">Rapor PTS ${escapeHtml(raporState.kelas)}</div><div class="rp-muted">${raporState.siswa.length} siswa · nama file PDF menggunakan nama lengkap siswa.</div></div>
        <button class="btn rp-zip-btn" id="rp-zip-btn" onclick="downloadRaporPTSClassZip()">Download Semua (ZIP)</button>
      </div>
      <div class="rp-student-list">
        ${raporState.siswa.map((s,i)=>`<div class="rp-student-row">
          <div class="rp-no">${i+1}</div>
          <div class="rp-student-info"><strong>${escapeHtml(s.nama)}</strong><span>NIS ${escapeHtml(s.nis || '-')}</span></div>
          <button class="btn btn-outline rp-one-btn" onclick="downloadRaporPTSStudent('${escapeHtml(String(s.nis)).replace(/'/g,"\\'")}')">Download PDF</button>
        </div>`).join('')}
      </div>
    </div>`;
}

function triggerRaporDownload(url, filename){
...
async function downloadRaporPTSStudent(nis){
  const siswa = raporState.siswa.find(s => String(s.nis) === String(nis));
  if(!siswa) return;
  showToast(`Menyiapkan rapor ${siswa.nama}...`);
  try{
    const res = await callApi('generateRaporPTSSiswa', {
      kelas:raporState.kelas, nis:siswa.nis, periode:periodeKodeRapor(), requestedBy:currentUser.nama
    });
    if(!res.success || !res.downloadUrl) throw new Error();
    triggerRaporDownload(res.downloadUrl, res.fileName || `${siswa.nama}.pdf`);
  }catch(err){
    showToast('Terjadi kendala. Silakan hubungi admin.', true);
  }
}

async function downloadRaporPTSClassZip(){
  const btn = document.getElementById('rp-zip-btn');
  if(btn){ btn.disabled=true; btn.innerHTML='<span class="spinner"></span>Menyiapkan ZIP...'; }
  try{
    const res = await callApi('generateRaporPTSKelasZip', {
      kelas:raporState.kelas, periode:periodeKodeRapor(), requestedBy:currentUser.nama
    });
    if(!res.success || !res.downloadUrl) throw new Error();
    triggerRaporDownload(res.downloadUrl, res.fileName || `Rapor PTS - ${raporState.kelas}.zip`);
    showToast('ZIP rapor kelas siap diunduh.');
  }catch(err){
    showToast('Terjadi kendala. Silakan hubungi admin.', true);
  }finally{
    if(btn){ btn.disabled=false; btn.textContent='Download Semua (ZIP)'; }
  }
}
...
async function loadDashboardWalasFast(requestToken){
  ...
  try{
    ...
    root.innerHTML += `<div class="card"><div class="empty-state"><div class="icon">—</div>Terjadi kendala. Silakan hubungi admin.</div></div>`;
...
/* ==========================================================
   CQLASS V3 — SIMPLE CASE AI
   ========================================================== */
function renderHasilMasalahAI(siswa, tanggal, cerita, tindakan, risiko){
  const ai = masalahState.analisisAI || {};
  const result = document.getElementById('ms-ai-result');
  const urgent = Boolean(ai.mendesak) || risiko === 'Ya';
  const saran = Array.isArray(ai.saran) ? ai.saran : (Array.isArray(ai.penangananLangsung) ? ai.penangananLangsung : []);
  const koordinasi = ai.koordinasi || ai.saranOrangTua || '';
  const finalText = [
    ai.ringkasan ? `Ringkasan: ${ai.ringkasan}` : '',
    saran.length ? `Saran:\n${saran.map(x=>'- '+x).join('\n')}` : '',
  ...
  result.innerHTML = `
    <div class="card">
      ${urgent ? `<div class="ms-alert"><strong>Perlu perhatian segera.</strong><br>Utamakan keselamatan siswa dan ikuti prosedur sekolah.</div>` : ''}
      <div class="ms-ai-title" style="margin-bottom:12px">Saran Pendampingan</div>
      <div class="ms-ai-section"><h4>Ringkasan</h4><p>${escapeHtml(ai.ringkasan || '-')}</p></div>
      <div class="ms-ai-section"><h4>Saran</h4>${renderListMasalah(saran)}</div>
      ${koordinasi ? `<div class="ms-ai-section"><h4>Koordinasi</h4><p>${escapeHtml(koordinasi)}</p></div>` : ''}
      <div class="ms-final-box">
        <div class="ms-field" style="margin:0">
          <label>Catatan final wali kelas</label>
          <textarea id="ms-saran-final" style="min-height:180px">${escapeHtml(finalText)}</textarea>
          <div class="ms-help">Wali kelas dapat mengedit sebelum menyimpan.</div>
        </div>
      </div>
      <div class="ms-actions">
        <button class="btn" id="ms-save-btn" onclick="saveCatatanMasalah()">Simpan Laporan</button>
...
/* Nilai akademik & vocabulary masih bersumber dari spreadsheet eksternal.
   CQlass tidak mengubah sumber inputnya pada tahap ini. */

/* ==========================================================
   CQLASS FINAL — INPUT KETERLAMBATAN WALAS
   ========================================================== */
function renderKdInputKeterlambatan(){
  const c = document.getElementById('kd-tab-content');
  const options = kdState.siswa.map(s => `<option value="${escapeHtml(s.nis)}">${escapeHtml(s.nama)} — ${escapeHtml(s.nis)}</option>`).join('');
  c.innerHTML = `
    <div class="card">
      <div class="card-title">Catat Keterlambatan yang Belum Dicatat Pengabdian</div>
      <div style="font-size:12px;color:var(--muted);line-height:1.6;margin-bottom:16px;">
        Sistem akan mengecek NIS + tanggal terlebih dahulu. Jika sudah tercatat dari web pengabdian, data tidak akan dibuat dua kali.
      </div>
      <div class="ms-grid">
        <div class="ms-field"><label>Siswa</label><select id="kd-telat-siswa"><option value="">— pilih siswa —</option>${options}</select></div>
        <div class="ms-field"><label>Tanggal</label><input type="date" id="kd-telat-tanggal" value="${todayStrKD()}"></div>
      </div>
      <div class="ms-grid">
        <div class="ms-field"><label>Jam Datang</label><input type="time" id="kd-telat-jam"></div>
        <div class="ms-field"><label>Alasan <span style="font-weight:400">(opsional)</span></label><input id="kd-telat-alasan" placeholder="Contoh: kendaraan terlambat"></div>
      </div>
      <button class="btn" id="kd-telat-save" onclick="simpanKeterlambatanWalas()">Simpan Keterlambatan</button>
    </div>`;
}
async function simpanKeterlambatanWalas(){
  const nis=document.getElementById('kd-telat-siswa')?.value||'';
  const s=kdState.siswa.find(x=>String(x.nis)===String(nis));
  const tanggal=document.getElementById('kd-telat-tanggal')?.value||'';
  if(!s||!tanggal){showToast('Pilih siswa dan tanggal terlebih dahulu.',true);return}
  const b=document.getElementById('kd-telat-save');b.disabled=true;b.innerHTML='<span class="spinner"></span>Menyimpan...';
  try{
    const r=await callApi('catatKeterlambatanWalas',{kelas:kdState.kelas,nis:s.nis,nama:s.nama,tanggal,
      jam:document.getElementById('kd-telat-jam')?.value||'',alasan:document.getElementById('kd-telat-alasan')?.value||'',
      dicatatOleh:currentUser.nama});
    if(r.duplicate){showToast('Keterlambatan siswa ini sudah tercatat.');}
    else{showToast('Keterlambatan berhasil dicatat.');}
    await loadKdKeterlambatanPending();renderKdNotifKeterlambatan();
  }catch(e){showToast('Terjadi kendala. Silakan hubungi admin.',true)}
  finally{b.disabled=false;b.textContent='Simpan Keterlambatan'}
}

/* ==========================================================
   CQLASS FINAL — ADMIN TANDA TANGAN
   ========================================================== */
let ttdAdminState={data:null,fileBase64:null,mimeType:null,fileName:''};
async function renderTTDAdmin(content){
  content.innerHTML=`<div class="page-title">Tanda Tangan</div><div class="page-sub">Kelola tanda tangan Kepala Sekolah dan Wali Kelas untuk dokumen resmi.</div><div id="ttd-admin-main" class="card"><span class="spinner" style="border-top-color:var(--primary);border-color:rgba(10,110,110,.25)"></span>Memuat...</div>`;
  try{const r=await callApi('getTTDAdminData');ttdAdminState.data=r.data;drawTTDAdmin();}
  catch(e){document.getElementById('ttd-admin-main').innerHTML='<div class="empty-state">Terjadi kendala. Silakan hubungi admin.</div>'}
}
function drawTTDAdmin(){
  const d=ttdAdminState.data||{}, main=document.getElementById('ttd-admin-main');
  const walas=(d.walas||[]).map(x=>`<option value="${escapeHtml(x.nama)}">${escapeHtml(x.nama)} — ${escapeHtml(x.kelas||'-')}</option>`).join('');
  main.innerHTML=`
    <div class="card-title">Upload Tanda Tangan</div>
    <div class="ms-grid">
      <div class="ms-field"><label>Jenis</label><select id="ttd-jenis" onchange="ttdJenisChanged()"><option value="kepsek">Kepala Sekolah</option><option value="walas">Wali Kelas</option></select></div>
      <div class="ms-field" id="ttd-target-wrap"><label>Nama Kepala Sekolah</label><input id="ttd-kepsek-nama" value="${escapeHtml(d.kepsek?.nama||'')}"></div>
    </div>
    <div class="ms-field"><label>File Tanda Tangan</label><input type="file" id="ttd-file" accept="image/png,image/jpeg,image/webp" onchange="ttdFileSelect(event)"><div class="ms-help">Disarankan PNG transparan.</div></div>
    <button class="btn" id="ttd-save" onclick="saveTTDAdmin()">Upload & Simpan</button>
    <div style="margin-top:18px;font-size:12px;color:var(--muted)">Folder Drive: ${escapeHtml(d.folderName||'CQlass - Tanda Tangan')}</div>`;
  ttdAdminState.walasOptions=walas;
}
function ttdJenisChanged(){
  const jenis=document.getElementById('ttd-jenis').value,w=document.getElementById('ttd-target-wrap');
  if(jenis==='walas')w.innerHTML=`<label>Wali Kelas</label><select id="ttd-walas-nama"><option value="">— pilih wali kelas —</option>${ttdAdminState.walasOptions||''}</select>`;
  else w.innerHTML=`<label>Nama Kepala Sekolah</label><input id="ttd-kepsek-nama" value="${escapeHtml(ttdAdminState.data?.kepsek?.nama||'')}">`;
}
function ttdFileSelect(e){
  const f=e.target.files?.[0];if(!f)return;ttdAdminState.mimeType=f.type;ttdAdminState.fileName=f.name;
  const r=new FileReader();r.onload=()=>{ttdAdminState.fileBase64=String(r.result).split(',')[1]||''};r.readAsDataURL(f);
}
async function saveTTDAdmin(){
  if(!ttdAdminState.fileBase64){showToast('Pilih file tanda tangan terlebih dahulu.',true);return}
  const jenis=document.getElementById('ttd-jenis').value;
  const target=jenis==='walas'?(document.getElementById('ttd-walas-nama')?.value||''):(document.getElementById('ttd-kepsek-nama')?.value||'');
  if(!target){showToast('Pilih/isi nama terlebih dahulu.',true);return}
  const b=document.getElementById('ttd-save');b.disabled=true;b.innerHTML='<span class="spinner"></span>Menyimpan...';
  try{
    const r=await callApi('uploadTTD',{jenis,target,base64:ttdAdminState.fileBase64,mimeType:ttdAdminState.mimeType,fileName:ttdAdminState.fileName});
    if(!r.success)throw new Error();showToast('Tanda tangan berhasil disimpan.');ttdAdminState.fileBase64=null;
    const d=await callApi('getTTDAdminData');ttdAdminState.data=d.data;drawTTDAdmin();
  }catch(e){showToast('Terjadi kendala. Silakan hubungi admin.',true)}
  finally{if(document.getElementById('ttd-save')){b.disabled=false;b.textContent='Upload & Simpan'}}
}
