...
function patchText(){
  document.querySelectorAll('body *').forEach(el=>{
    ...
    else if(s==='Belum ada entry mengajar/JP pada periode ini.')el.textContent='Semua slot jam kosong sudah terisi penuh pada periode ini.';
  });
}
...
function patchTimesheetPopup(){
  ...
  groups.forEach(g=>{
    ...
    if(/Mengajar\s*\/\s*JP/i.test(txt)){
      if(h)h.textContent='Jam kosong belum terisi penuh';
      g.classList.remove('done');g.classList.add('todo');
      g.querySelectorAll('.cqts-empty').forEach(x=>{if(/mengajar|JP/i.test(x.textContent||''))x.textContent='Semua slot jam kosong sudah terisi penuh pada periode ini.'});
