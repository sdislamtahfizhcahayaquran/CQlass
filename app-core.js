...
const TEACHER_SIGNATURE_URL = `${SUPABASE_URL}/functions/v1/teacher-signature`;
...
/* ==========================================================
   TANDA TANGAN RAPOR — WALAS / KEPALA SEKOLAH
   - Menempel langsung ke tab Foto Profil; tidak meng-hook login.
   - JPG/JPEG/PNG -> remove background putih -> PNG transparan.
   - Storage private melalui Edge Function teacher-signature.
   ========================================================== */
let teacherSignatureState={pngDataUrl:'',loading:false};

async function teacherSignatureRequest(method='GET',payload=null,timeoutMs=30000){
  const token=getAuthToken();
  if(!token)throw new Error('Sesi login tidak ditemukan. Silakan login ulang.');
  const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),timeoutMs);
  try{
    const response=await fetch(TEACHER_SIGNATURE_URL,{
      method,
      headers:{
        'Content-Type':'application/json',
        'apikey':SUPABASE_PUBLISHABLE_KEY,
    ...
    try{data=raw?JSON.parse(raw):{}}catch(_){throw new Error(`Respons tanda tangan bukan JSON. HTTP ${response.status}.`)}
    if(!response.ok||data.ok===false||data.success===false)throw new Error(data.error||`Gagal memproses tanda tangan (HTTP ${response.status}).`);
    return data;
  }catch(err){
...
function signatureRoleAllowed(){
  return ['walas','pimpinan','admin'].includes(String(currentUser?.role||'').toLowerCase());
}

function renderTeacherSignaturePanel(){
  const pane=document.getElementById('pane-foto');
  if(!pane||!signatureRoleAllowed())return;
  let box=document.getElementById('teacher-signature-panel');
  if(!box){
    box=document.createElement('div');
    box.id='teacher-signature-panel';
    box.style.cssText='margin-top:18px;padding-top:16px;border-top:1px solid var(--border,#dbe5e4)';
    box.innerHTML=`
      <div style="font-size:13px;font-weight:900;color:var(--text,#183332);margin-bottom:4px">Tanda Tangan Rapor</div>
      <div style="font-size:11px;color:var(--muted,#70817f);line-height:1.5;margin-bottom:10px">Upload foto tanda tangan di kertas putih. Sistem otomatis menghapus background dan menyimpan hasil sebagai PNG transparan.</div>
      <input id="teacher-signature-file" type="file" accept="image/jpeg,image/png" style="display:none" onchange="handleTeacherSignatureSelect(event)">
      <button type="button" class="btn btn-sm" onclick="document.getElementById('teacher-signature-file').click()">Pilih Foto Tanda Tangan</button>
      <div id="teacher-signature-preview" style="margin-top:10px;min-height:120px;border:1px dashed var(--border,#dbe5e4);border-radius:11px;background:#f8faf9;display:flex;align-items:center;justify-content:center;padding:10px">
        <span style="font-size:11px;color:var(--muted,#70817f)">Memuat tanda tangan...</span>
      </div>
      <div id="teacher-signature-msg" style="min-height:18px;margin-top:7px;font-size:11px;color:var(--muted,#70817f)"></div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px">
        <button type="button" class="btn btn-sm" id="teacher-signature-save" onclick="saveTeacherSignature()" disabled>Simpan Tanda Tangan</button>
        <button type="button" class="btn btn-sm" id="teacher-signature-delete" onclick="deleteTeacherSignature()">Hapus</button>
      </div>`;
    pane.appendChild(box);
  }
  loadTeacherSignaturePreview();
}

async function loadTeacherSignaturePreview(){
  const preview=document.getElementById('teacher-signature-preview'),msg=document.getElementById('teacher-signature-msg');
  if(!preview)return;
  teacherSignatureState.pngDataUrl='';
  const save=document.getElementById('teacher-signature-save');if(save)save.disabled=true;
  preview.innerHTML='<span class="spinner"></span>';
  try{
    const data=await teacherSignatureRequest('GET');
    if(data.exists&&data.url){
      preview.innerHTML=`<img src="${escapeHtml(data.url)}" alt="Tanda tangan tersimpan" crossorigin="anonymous" style="max-width:95%;max-height:105px;object-fit:contain">`;
      if(msg)msg.textContent='Tanda tangan sudah tersimpan. Pilih foto baru jika ingin mengganti.';
    }else{
      preview.innerHTML='<span style="font-size:11px;color:var(--muted,#70817f)">Belum ada tanda tangan tersimpan.</span>';
...
function processTeacherSignatureImage(file){
  return new Promise((resolve,reject)=>{
    ...
    if(!['image/jpeg','image/png'].includes(file.type))return reject(new Error('Format harus JPG, JPEG, atau PNG.'));
    if(file.size>8*1024*1024)return reject(new Error('Foto awal maksimal 8 MB.'));
    const reader=new FileReader();
...
async function handleTeacherSignatureSelect(e){
  const file=e.target.files?.[0],preview=document.getElementById('teacher-signature-preview'),msg=document.getElementById('teacher-signature-msg'),save=document.getElementById('teacher-signature-save');
  if(!file)return;
  try{
    if(msg)msg.textContent='Memproses foto dan menghapus background...';
    const png=await processTeacherSignatureImage(file);teacherSignatureState.pngDataUrl=png;
    if(preview)preview.innerHTML=`<img src="${png}" alt="Preview tanda tangan" style="max-width:95%;max-height:105px;object-fit:contain">`;
    if(save)save.disabled=false;
    if(msg)msg.textContent='Preview siap. Klik Simpan Tanda Tangan.';
  }catch(err){teacherSignatureState.pngDataUrl='';if(save)save.disabled=true;if(msg)msg.textContent=err?.message||'Gagal memproses gambar.';showToast(err?.message||'Gagal memproses tanda tangan',true)}
}

async function saveTeacherSignature(){
  if(!teacherSignatureState.pngDataUrl){showToast('Pilih foto tanda tangan terlebih dahulu.',true);return}
  const btn=document.getElementById('teacher-signature-save'),msg=document.getElementById('teacher-signature-msg');
  const old=btn?.textContent||'Simpan Tanda Tangan';if(btn){btn.disabled=true;btn.innerHTML='<span class="spinner"></span>Menyimpan...'}
  try{
    const base64=teacherSignatureState.pngDataUrl.split(',')[1]||'';
    await teacherSignatureRequest('POST',{png_base64:base64},45000);
    teacherSignatureState.pngDataUrl='';if(msg)msg.textContent='Tanda tangan berhasil disimpan.';showToast('Tanda tangan berhasil disimpan');
    await loadTeacherSignaturePreview();
  }catch(err){if(msg)msg.textContent=err?.message||'Gagal menyimpan tanda tangan.';showToast(err?.message||'Gagal menyimpan tanda tangan',true)}
  finally{if(btn){btn.disabled=!teacherSignatureState.pngDataUrl;btn.textContent=old}}
}

async function deleteTeacherSignature(){
  if(!confirm('Hapus tanda tangan yang tersimpan?'))return;
  const msg=document.getElementById('teacher-signature-msg');
  try{await teacherSignatureRequest('DELETE',null,30000);teacherSignatureState.pngDataUrl='';showToast('Tanda tangan berhasil dihapus');await loadTeacherSignaturePreview()}
  catch(err){if(msg)msg.textContent=err?.message||'Gagal menghapus tanda tangan.';showToast(err?.message||'Gagal menghapus tanda tangan',true)}
}
...
function rpApplySignatureBundle(report,bundle){
  ...
  report.teachers=report.teachers||{};
  if(bundle.principal_name&&!String(report.teachers.principal||'').trim())report.teachers.principal=bundle.principal_name;
  if(bundle.homeroom_name&&!String(report.teachers.homeroom||'').trim())report.teachers.homeroom=bundle.homeroom_name;
  report.teachers.principal_signature_url=bundle.principal_signature_url||report.teachers.principal_signature_url||'';
  report.teachers.homeroom_signature_url=bundle.homeroom_signature_url||report.teachers.homeroom_signature_url||'';
  return report;
}
async function rpWaitForImages(root){
  const imgs=[...(root?.querySelectorAll?.('img')||[])];
  await Promise.all(imgs.map(img=>{
    if(img.complete)return img.decode?.().catch(()=>{})||Promise.resolve();
    return new Promise(resolve=>{img.addEventListener('load',resolve,{once:true});img.addEventListener('error',resolve,{once:true});setTimeout(resolve,8000)});
  }));
}
...
function enterApp(){
  ...
  if(String(currentUser?.role||'').trim().toLowerCase()==='hrd'){
    ...
    if(typeof window.openHrdCleanDashboard==='function'){
      ...
      sc.src='hrd-role-cleanup.js?v=20260925-mobiledata8';
...
function rpEkskulRemarkWithContext(kind,grade,eks){const base=rpEkskulRemark(kind,grade);if(kind==='activity'&&String(eks?.external_activity||'').trim())return String(eks.external_activity).trim()+' · '+base;if(kind==='competition'&&String(eks?.competition_note||'').trim())return String(eks.competition_note).trim();return base}
...
async function renderCetakRapor(content){
  ...
  raporPreviewState={academicYear:'',semester:0,reportType:'PTS',classes:[],classLocked:false,classId:'',students:[],studentId:'',startDate:rpDefaultStartYmd(),endDate:rpTodayYmd(),printDate:rpTodayYmd(),hijriDate:'',report:null,classReports:[]};
  ...
  try{const b=await reportPreviewRequest('bootstrap',{});raporPreviewState.academicYear=b.academic_year||'';raporPreviewState.semester=Number(b.semester_no)||0;raporPreviewState.classes=b.classes||[];raporPreviewState.classLocked=Boolean(b.class_locked);raporPreviewState.classId=b.default_class_id||'';renderRaporControls();if(raporPreviewState.classId)await rpLoadStudents()}catch(e){document.getElementById('rpv-root').innerHTML=`<div class="card"><div class="ms-alert">${escapeHtml(e.message||'Gagal membuka Rapor.')}</div></div>`}
}
...
async function rpLoadPreview(){
  ...
  try{
    ...
    raporPreviewState.report=d.report;const sig=await rpFetchReportSignatures(classId);rpApplySignatureBundle(raporPreviewState.report,sig);renderRaporPreview();return true;
...
function rpReportGrade(){const r=raporPreviewState?.report||{};const n=Number(r.class?.grade_level);if(Number.isFinite(n)&&n>0)return n;const m=String(r.class?.name||'').match(/^(\d+)/);return m?Number(m[1]):null}
function rpAcademicRows(rows){
  const list=Array.isArray(rows)?rows:[],grade=rpReportGrade();
  return RP_SUBJECT_TEMPLATE.map(slot=>{
    if(slot.group){const groupNo=slot.group==='Islamic Studies'?1:9;return`<tr class="rpv-group-row"><td class="rpv-center">${groupNo}</td><td colspan="8">${escapeHtml(slot.group)}</td></tr>`}
    const isSub=!slot.no,no=slot.no||'',r=rpFindAcademic(list,slot.aliases)||{};
...
function rpTeamRows(team,tahfizh,homeroom){const arr=rpBuildTeamList(team,tahfizh,homeroom);if(!arr.length)return'<tr><td>1.</td><td>-</td></tr>';return arr.map((x,i)=>`<tr><td>${i+1}.</td><td>${escapeHtml(x.name)}</td></tr>`).join('')}
function rpTeamPositions(team,tahfizh,homeroom){const arr=rpBuildTeamList(team,tahfizh,homeroom);return arr.length?arr.map(x=>`<div>${escapeHtml(x.pos)}</div>`).join(''):'<div>-</div>'}
...
function renderRaporPreview(){
  ...
  const st=r.student||{},cl=r.class||{},att=r.attendance||{},pts=r.points||{},eks=r.extracurricular||{},trs=r.teachers||{};const p=att.percent||{},cats=pts.categories||{};
  ...
  const semTxt=Number(r.semester_no)===2?'SEMESTER II':'SEMESTER I',idText=[st.nis,st.nisn].filter(Boolean).join(' / ')||'-',t=r.tahfizh||null;
  const tahRows=t?`
    <tr><td>Memorization Material</td><td>:</td><td>${escapeHtml(rpEnglishRange(t.material)||'-')}</td></tr><tr><td>Tahfizh Learning Target (LP)</td><td>:</td><td>${escapeHtml(rpEnglishRange(t.target)||'-')}</td></tr><tr><td>Current Achievement</td><td>:</td><td>${escapeHtml(t.current||'-')}</td></tr><tr><td>Tahfizh Achievement</td><td>:</td><td>${escapeHtml(t.achievement||'-')}</td></tr><tr><td class="sub">a. Number of Surahs</td><td>:</td><td>${escapeHtml(t.surahs??'-')}</td></tr><tr><td class="sub">b. Number of Lines</td><td>:</td><td>${escapeHtml(t.lines??'-')}</td></tr><tr><td class="sub">c. Number of Verses</td><td>:</td><td>${escapeHtml(t.verses??'-')}</td></tr><tr><td class="sub">d. Percentage (%)</td><td>:</td><td>${escapeHtml(t.percentage??'-')}</td></tr><tr><td>Juz Advancement Assessment</td><td>:</td><td>${escapeHtml(t.juz_assessment||'-')}</td></tr>`:`
  ...
  const principalImg=trs.principal_signature_url?`<img class="rpv-signature-img" crossorigin="anonymous" src="${escapeHtml(trs.principal_signature_url)}" alt="Tanda tangan Kepala Sekolah">`:'';
  const homeroomImg=trs.homeroom_signature_url?`<img class="rpv-signature-img" crossorigin="anonymous" src="${escapeHtml(trs.homeroom_signature_url)}" alt="Tanda tangan Wali Kelas">`:'';
  ...
  <div class="rpv-paper-wrap" id="rpv-preview">
    ...
    <section class="rpv-paper">
      ...
      <table class="rpv-table rpv-exkul"><thead><tr><th style="width:5%">No.</th><th style="width:50%">Extracurricular &amp; Personal Development</th><th style="width:8%">Rating</th><th>Remarks</th></tr></thead><tbody><tr><td class="rpv-center">1</td><td>Extracurricular Participation</td><td class="rpv-center"><b>${rpGrade(eks.activity_grade)}</b></td><td>${escapeHtml(rpEkskulRemarkWithContext('activity',eks.activity_grade,eks))}</td></tr><tr><td class="rpv-center">2</td><td>Extracurricular Skill Development</td><td class="rpv-center"><b>${rpGrade(eks.skill_grade)}</b></td><td>${escapeHtml(rpEkskulRemarkWithContext('skill',eks.skill_grade,eks))}</td></tr><tr><td class="rpv-center">3</td><td>Competition Participation and Achievement</td><td class="rpv-center"><b>${rpGrade(eks.competition_grade)}</b></td><td>${escapeHtml(rpEkskulRemarkWithContext('competition',eks.competition_grade,eks))}</td></tr><tr><td class="rpv-center">4</td><td>Participation in School Activities</td><td class="rpv-center"><b>${rpGrade(eks.school_activity_grade)}</b></td><td>${escapeHtml(rpEkskulRemarkWithContext('school',eks.school_activity_grade,eks))}</td></tr></tbody></table>
      <div class="rpv-p2-section" style="margin-top:17px">5.&nbsp;&nbsp;Student Character and Discipline Report</div>
      <div class="rpv-p2-sub">A. Student Merit and Guidance Record</div><table class="rpv-table rpv-merit"><thead><tr><th style="width:51%">Category</th><th style="width:14%">Points</th><th>Remarks</th></tr></thead><tbody><tr><td>Achievement/Role Model Award</td><td class="rpv-center">${pts.reward_total||0}</td><td>-</td></tr><tr><td>Points Deducted through Guidance</td><td class="rpv-center">${pts.violation_total||0}</td><td>-</td></tr><tr><td>Final Total Points: <b>${pts.final_total||0}</b> Points</td><td></td><td></td></tr></tbody></table>
      <div class="rpv-p2-sub" style="margin-top:14px">B. Disciplinary Record Summary</div><table class="rpv-table rpv-discipline"><thead><tr><th style="width:51%">Level</th><th style="width:14%">Number of<br>Incidents</th><th>Remarks</th></tr></thead><tbody><tr><td>Minor</td><td class="rpv-center">${cats.Minor?.incidents||0}</td><td>-</td></tr><tr><td>Moderate</td><td class="rpv-center">${cats.Moderate?.incidents||0}</td><td>-</td></tr><tr><td>Severe</td><td class="rpv-center">${cats.Severe?.incidents||0}</td><td>-</td></tr><tr><td>Total Violation Points: <b>${pts.violation_total||0}</b> Points</td><td></td><td></td></tr></tbody></table>
  ...
  const root=document.getElementById('rpv-preview');rpWaitForImages(root).finally(()=>root?.scrollIntoView({behavior:'smooth',block:'start'}));
}
