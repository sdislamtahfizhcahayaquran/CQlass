...
(function(){
  ...
  /* Bust the visual stylesheet cache without changing the rest of index.html. */
  const css=[...document.querySelectorAll('link[rel="stylesheet"]')].find(x=>/login-redesign\.css/i.test(x.getAttribute('href')||''));
  if(css) css.href='login-redesign.css?v=20260909-exact2';

  /* Replace only the contents of #login-screen. app.js/app-core.js still owns login behavior. */
  ...
  if(login){
          ...
          <div class="left">
            ...
            <div class="features">
              <div class="feature-card">
                ...
                <div><strong>Akademik</strong><span>Kelola pembelajaran dan penilaian</span></div>
              </div>
              <div class="feature-card">
                <div class="icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg></div>
                <div><strong>Kesiswaan</strong><span>Data siswa, absensi, dan perkembangan</span></div>
              </div>
              <div class="feature-card">
                ...
                <div><strong>Tahfizh &amp; Kegiatan</strong><span>Pantau capaian tahfizh dan aktivitas sekolah</span></div>
              </div>
            </div>

            <blockquote class="quote">
              "Pendidikan hari ini, cahaya untuk masa depan yang lebih baik."
              <cite>— SDIT Cahaya Qur'an</cite>
            </blockquote>
          </div>
            ...
            <div class="login-card">
              ...
              <form id="login-form" novalidate>
                ...
                <div class="field" id="passField">
                  ...
                  <button type="button" class="icon-right toggle-pass" id="toggle-pass" aria-label="Tampilkan kata sandi">
                    <svg id="toggle-pass-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-7 11-7 11 7 11 7-4 7-11 7-11-7-11-7z"/><circle cx="12" cy="12" r="3"/></svg>
                  </button>
                  <div class="field-msg">Kata sandi tidak boleh kosong.</div>
                </div>

                <div class="row-between">
                  <label class="remember"><input type="checkbox" id="login-remember"> Ingat saya</label>
              ...
              <div class="contact-line">
                ...
                Hubungi admin sekolah jika mengalami kendala login.
              </div>

              <div class="mosque-footer">
                <svg viewBox="0 0 200 90" fill="none" stroke="currentColor" stroke-width="2.5">
                  <path d="M20 90V55c0-8 6-14 14-14s14 6 14 14v35"/>
                  <path d="M152 90V55c0-8 6-14 14-14s14 6 14 14v35"/>
                  <path d="M70 90V40c0-16 12-28 30-28s30 12 30 28v50"/>
                  <circle cx="100" cy="8" r="3"/>
                  <path d="M100 4V-2" transform="translate(0,4)"/>
                </svg>
                <p>TEKNOLOGI UNTUK PENDIDIKAN<br>YANG LEBIH BAIK</p>
              </div>
            </div>
          </div>
        </main>
    ...
    /* Same local empty-field feedback as the supplied visual; does not intercept valid submit. */
  ...
  /* These are the approved CQlass visual assets already stored in the repo's preview branch. */
  ...
  const load=async(names)=>{
    const parts=await Promise.all(names.map(async name=>{
      const r=await fetch(BASE+name+'?v=20260909-exact2',{cache:'force-cache'});
