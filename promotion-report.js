/* CQlass — Promosi Socmed */
(function(){
'use strict';
if(window.__cqPromotionReportV8)return;window.__cqPromotionReportV8=true;
...
const BASE_ROLES=['admin','guru','walas','wali_kelas','pimpinan','kepsek','kepala_sekolah','akademik','academic','kesiswaan','tahfizh','kegiatan','guru_partner','partner','kabid_akademik','kabid_kesiswaan','kabid_tahfizh','kabid_kegiatan'];
...
async function load(){
  ...
  try{
    ...
    status.className='pr-status '+(d.fulfilled?'ok':'bad');status.textContent=d.fulfilled?'✓ Kewajiban bulan ini sudah terpenuhi.':'Belum memenuhi: unggah minimal 1 foto Promosi Socmed bulan ini.';if(count)count.textContent=items.length;
    box.innerHTML=items.length?items.map(x=>`<article class="pr-item"><a href="${esc(x.photo_url||'#')}" target="_blank" rel="noopener"><img src="${esc(x.photo_url||'')}" alt="Promosi Socmed" onerror="this.style.opacity='.25'"></a><div><b>${esc(x.title||'Promosi Socmed')}</b><span>${esc(x.description||'Tanpa keterangan')}</span><small>${fmt(x.activity_date)}</small></div><button class="pr-delete" type="button" data-del="${esc(x.id)}">Hapus</button></article>`).join(''):'<div class="pr-empty">Belum ada foto Promosi Socmed pada bulan ini.</div>';
...
function render(){
  ...
  if(role()==='hrd'){c.innerHTML='<div class="pr-shell"><section class="pr-card"><h2>Promosi Socmed dipantau dari Live Report HRD</h2><p>Halaman input Promosi tidak digunakan oleh HRD.</p></section></div>';return;}
  ...
  c.innerHTML=`<div class="pr-shell"><section class="pr-hero"><small>PROMOSI SOCMED</small><h1>Promosi Socmed</h1><p>Setiap pegawai wajib mengunggah minimal satu foto Promosi Socmed pada periode laporan.</p></section><div class="pr-grid"><section class="pr-card"><h2>Tambah Bukti</h2><p>Satu kegiatan menggunakan satu foto yang jelas.</p><div id="pr-status" class="pr-status wait">Memeriksa status…</div><form class="pr-form" id="pr-form" style="margin-top:12px"><label>Tanggal kegiatan<input id="pr-date" type="date" value="${today()}" required></label><label>Judul kegiatan<input id="pr-title" maxlength="120" value="Promosi Socmed" required></label><label>Keterangan singkat<textarea id="pr-desc" rows="3" maxlength="500" placeholder="Contoh: Membagikan informasi PPDB melalui status WhatsApp"></textarea></label><label>Foto Promosi Socmed (wajib)<input id="pr-file" type="file" accept="image/jpeg,image/png,image/webp" required></label><img id="pr-preview" class="pr-preview" alt="Pratinjau foto"><button id="pr-save" type="submit">Simpan Foto Promosi</button></form></section><section class="pr-card"><div class="pr-list-head"><div><h2>Riwayat Bulan Ini</h2><p style="margin:3px 0 0;font-size:11px;color:#6b8080">Foto yang tersimpan otomatis masuk ke Live Report HRD.</p></div><span class="pr-count" id="pr-count">0</span></div><div id="pr-list" class="pr-list"></div></section></div></div>`;
  ...
  document.getElementById('pr-form').onsubmit=async e=>{e.preventDefault();if(!encoded)return notify('Pilih foto Promosi Socmed terlebih dahulu.',true);const b=document.getElementById('pr-save');b.disabled=true;b.textContent='Menyimpan…';try{await api({action:'upload',activity_date:document.getElementById('pr-date').value,title:document.getElementById('pr-title').value,description:document.getElementById('pr-desc').value,mime_type:'image/jpeg',base64:encoded});notify('Foto Promosi Socmed berhasil disimpan.');render()}catch(err){notify(err.message||'Gagal menyimpan foto Promosi Socmed.',true);b.disabled=false;b.textContent='Simpan Foto Promosi'}};
  load();
}
async function remove(id){if(!id||!confirm('Hapus foto Promosi Socmed ini?'))return;try{await api({action:'delete',id});notify('Foto Promosi Socmed dihapus.');await load()}catch(e){notify(e.message||'Gagal menghapus foto Promosi Socmed.',true)}}
function install(){
  if(typeof MODULE_GROUPS==='undefined'||!Array.isArray(MODULE_GROUPS))return false;
  const rs=roles(),r=role();let g=MODULE_GROUPS.find(x=>x&&x.id==='laporan');
  if(!g){g={id:'laporan',label:'Laporan',roles:[],items:[]};MODULE_GROUPS.push(g)}if(!Array.isArray(g.items))g.items=[];
  g.roles=[...new Set([...(g.roles||[]),...rs])].filter(x=>!(r==='hrd'&&x==='hrd'));
  const obj={id:'laporan-promosi',label:'Promosi Socmed',roles:[...rs],built:true,render};
  const old=g.items.find(x=>x&&x.id===obj.id);if(old)Object.assign(old,obj);else g.items.push(obj);
  if(r==='hrd'){
    const it=g.items.find(x=>x&&x.id==='laporan-promosi');if(it)it.roles=(it.roles||[]).filter(x=>x!=='hrd');
  }
  return true;
}
function ensureSidebarEntry(){
  ...
  if(r==='sapras'||r==='hrd'){
    sb.querySelectorAll('[data-cq-promotion-fallback],.nav-item,.menu-item,button,a').forEach(x=>{if(String(x.textContent||'').trim()==='Promosi Socmed'||x.dataset?.cqPromotionFallback)x.remove()});return;
  }
}
function hookSidebar(){
  if(typeof renderSidebar!=='function'||renderSidebar.__cqPromotionV8)return;
  const old=renderSidebar;const wrapped=function(){install();const out=old.apply(this,arguments);requestAnimationFrame(ensureSidebarEntry);return out};wrapped.__cqPromotionV8=true;renderSidebar=wrapped;
}
...
function start(){
  ...
  ensureSidebarEntry();
  const sb=document.getElementById('sidebar');if(sb&&!sb.__cqPromotionObserver){sb.__cqPromotionObserver=true;new MutationObserver(ensureSidebarEntry).observe(sb,{childList:true,subtree:true})}
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(start,40),{once:true});else setTimeout(start,40);window.addEventListener('load',()=>setTimeout(start,0),{once:true});
})();
