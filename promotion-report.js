...
function render(){
  ...
  if(monitorOnly()){
    c.innerHTML=`<div class="pr-shell"><section class="pr-hero"><small>PROMOSI SOCMED</small><h1>Promosi Socmed</h1><p>Menu Promosi tersedia di semua role. HRD/Admin menggunakan halaman ini sebagai akses monitoring, bukan penginputan.</p></section><section class="pr-card pr-monitor"><strong>Mode monitoring</strong><p>Input foto Promosi Socmed dilakukan oleh pegawai pelaksana. HRD/Admin tetap memantau kelengkapan promosi melalui Live Report sehingga tidak ada input ganda dari role monitoring.</p></section></div>`;
    return;
  }
...
function install(){if(typeof MODULE_GROUPS==='undefined'||!Array.isArray(MODULE_GROUPS))return false;const rs=roles();let g=MODULE_GROUPS.find(x=>x&&x.id==='laporan');if(!g){g={id:'laporan',label:'Laporan',roles:[],items:[]};MODULE_GROUPS.push(g)}if(!Array.isArray(g.items))g.items=[];g.roles=[...new Set([...(g.roles||[]),...rs])];const obj={id:'laporan-promosi',label:'Promosi Socmed',roles:[...rs],built:true,render};const old=g.items.find(x=>x&&x.id===obj.id);if(old)Object.assign(old,obj);else g.items.push(obj);return true}
