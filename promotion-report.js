...
(function(){
'use strict';
if(window.__cqPromotionReportV6)return;window.__cqPromotionReportV6=true;
...
const ROLES=['guru','walas','pimpinan','kepsek','akademik','kesiswaan','tahfizh','kegiatan','sapras','guru_partner','kabid_akademik','kabid_kesiswaan','kabid_tahfizh','kabid_kegiatan'];
...
function install(){if(typeof MODULE_GROUPS==='undefined'||!Array.isArray(MODULE_GROUPS))return false;let g=MODULE_GROUPS.find(x=>x&&x.id==='laporan');if(!g){g={id:'laporan',label:'Laporan',roles:[],items:[]};MODULE_GROUPS.push(g)}g.roles=[...new Set([...(g.roles||[]),...ROLES])];const obj={id:'laporan-promosi',label:'Laporan Promosi',roles:[...ROLES],built:true,render};const old=g.items.find(x=>x&&x.id===obj.id);if(old)Object.assign(old,obj);else g.items.push(obj);return true}
window.renderPromotionReport=render;window.deletePromotionReport=remove;
function start(){if(install()&&typeof renderSidebar==='function')renderSidebar()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(start,150));else setTimeout(start,150);setTimeout(start,800);
})();
