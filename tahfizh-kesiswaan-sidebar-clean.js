...
(function(){
  'use strict';
  if(window.__cqTahfizhKesiswaanSidebarClean20260922V5)return;
  window.__cqTahfizhKesiswaanSidebarClean20260922V5=true;
  ...
  function ensureGroup(){
    ...
    // Akses generik di grup lain dicabut untuk Tahfizh agar tidak dobel.
    ...
    // ID khusus Tahfizh sengaja tidak memakai `kedisiplinan`/`reward` agar
    // kesiswaan-final-cleanup tidak menghapus submenu ini setelah render.
    const items=[
  ...
  function patch(){
    if(typeof renderSidebar==='function'&&!renderSidebar.__cqTahfizhKesiswaanV5){
      ...
      wrapped.__cqTahfizhKesiswaanV5=true;renderSidebar=wrapped;
    }
  }
  ...
  const attach=()=>{const sb=document.getElementById('sidebar');if(!sb||sb.__cqTahfizhKesiswaanObserverV5)return;sb.__cqTahfizhKesiswaanObserverV5=true;new MutationObserver(cleanDom).observe(sb,{childList:true,subtree:true})};
