...
(function(){
  'use strict';
  if(window.__CQ_ACADEMIC_PTS_READINESS_LIVE_V4__) return;
  window.__CQ_ACADEMIC_PTS_READINESS_LIVE_V4__=1;
  ...
  function css(){
    if(document.getElementById('akpts-css-v4'))return;
    const s=document.createElement('style');s.id='akpts-css-v4';s.textContent=`
      ...
      .akpts-unmapped{margin:0 18px 12px;padding:10px 12px;border-radius:12px;background:#fff8e8;border:1px solid #f2d694;color:#76591b;font-size:9px;line-height:1.5}.akpts-unmapped b{color:#5d4210}
      ...
      .akpts-wrap{overflow:auto;border-top:1px solid #edf3f6;max-height:590px}.akpts-table{width:100%;border-collapse:separate;border-spacing:0;min-width:1280px}.akpts-table th,.akpts-table td{padding:11px 10px;border-bottom:1px solid #edf2f4;text-align:left;vertical-align:middle;font-size:10px}.akpts-table th{background:#f5f9fb;color:#607680;text-transform:uppercase;letter-spacing:.025em;font-size:8.5px;position:sticky;top:0;z-index:3}.akpts-table tbody tr:nth-child(even){background:#fbfdfe}.akpts-table tbody tr:hover{background:#f1f8fc}.akpts-table tbody tr.akpts-row-ready{background:#f0fbf4}
  ...
  function classesRows(){
    ...
    if(!rows.length)return '<tr><td colspan="12"><div class="akpts-empty">Tidak ada kelas pada filter ini.</div></td></tr>';
    return rows.map(x=>{
      const att=x.attendance||{},attLabel=att.source_label||(att.source_mode==='editable'?'Rekap Editable':'Sistem');
      const attSub=att.source_mode==='editable'?`${attLabel} · ${att.validated_students||0}/${att.total_students||0}`:`${attLabel} · otomatis`;
      return `<tr class="${x.status==='siap'?'akpts-row-ready':''}">
      ...
      <td>${check(x.components?.attendance,attSub)}</td>
      <td>${check(x.components?.tahfizh,`${x.tahfizh?.complete_students||0}/${x.tahfizh?.total_students||0} siswa`)}</td>
      <td>${check(x.components?.extracurricular,`${x.extracurricular?.complete_students||0}/${x.extracurricular?.total_students||0} peserta`)}</td>
      <td>${check(x.components?.signature,x.components?.signature?'sudah upload':'belum upload')}</td>
  ...
  function walasRows(){
    ...
    if(!rows.length)return '<tr><td colspan="9"><div class="akpts-empty">Belum ada data wali kelas.</div></td></tr>';
    return rows.map(x=>`<tr><td><b>${E(x.teacher_name)}</b><div class="akpts-sub">${E(x.class_name)}</div></td><td>${check(x.components?.reward)}</td><td>${check(x.components?.discipline)}</td><td>${check(x.components?.activity)}</td><td>${check(x.components?.attendance)}</td><td>${check(x.components?.extracurricular)}</td><td>${check(x.components?.signature)}</td><td>${ratio(x.done_components,x.total_components)}</td><td><span class="akpts-status ${x.status==='selesai'?'ready':'pending'}">${x.status==='selesai'?'SELESAI':'BELUM SELESAI'}</span></td></tr>`).join('');
  }
  ...
  function table(){
    ...
    if(tab==='walas')return `<div class="akpts-wrap"><table class="akpts-table" style="min-width:980px"><thead><tr><th>Wali Kelas</th><th>Reward</th><th>Kedisiplinan</th><th>Kegiatan</th><th>Absensi</th><th>Ekskul</th><th>TTD Walas</th><th>Progres</th><th>Status</th></tr></thead><tbody>${walasRows()}</tbody></table></div>`;
    ...
    return `<div class="akpts-wrap"><table class="akpts-table"><thead><tr><th>Kelas</th><th>Walas</th><th>Nilai ≥2 TP</th><th>Reward</th><th>Kedisiplinan</th><th>Kegiatan</th><th>Absensi</th><th>Tahfizh</th><th>Ekskul</th><th>TTD Walas</th><th>Status Rapor</th><th>Yang Belum</th></tr></thead><tbody>${classesRows()}</tbody></table></div>`;
  }
  ...
  function render(){
    ...
    const unmappedText=unmapped.slice(0,6).map(x=>x.teacher_name||x.username||'Tanpa Nama').join(', ')+(unmapped.length>6?` +${unmapped.length-6}`:'');
    box.innerHTML=`
      ...
      ${table()}
      <div class="akpts-note">Aturan aktif: Nilai minimal 2 TP lengkap per mapel · Kegiatan minimal 1 · Absensi memakai Sistem secara default dan hanya menunggu validasi bila walas memilih Rekap Editable · Ekskul hanya menghitung peserta internal/eksternal · TTD wali kelas wajib. Reward/Kedisiplinan 0 tetap perlu konfirmasi nihil.</div>`;
  }
