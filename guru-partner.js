/* CQlass — Guru Partner: Absensi + Nilai Tahfizh berdasarkan halaqah */
(function(){
  ...
  const icon=n=>window.CQIcons?.svg?.(n)||'';
  ...
  function installRole(){
    ...
    DASHBOARD_MODULE.render=function(content){if(currentUser?.role==='partner')return renderPartnerDashboard(content);return oldDashRender(content)};
    if(!MODULE_GROUPS.some(g=>g.id==='partner-tasks')) MODULE_GROUPS.push({id:'partner-tasks',label:'Tugas Kelas',roles:['partner'],items:[
      {id:'absensi',label:'Absensi',roles:['partner'],built:true,render:renderAbsensi},
      {id:'partner-pts',label:'Nilai Tahfizh',roles:['partner'],built:true,render:renderPartnerPTS}
    ]});
  }
  ...
  async function detectPartner(){
    ...
    try{
      ...
      installRole();currentUser.role='partner';
      currentUser.kelas=(boot.classes||[])[0]?.name||currentUser.kelas||'';
      const roleEl=document.getElementById('user-role');if(roleEl)roleEl.textContent='GURU PARTNER';
      activeModule='dashboard';openGroupId=null;renderSidebar();setActiveModule('dashboard');
    }catch(err){if(err?.code!=='not_partner'&&err?.status!==403)console.warn('Partner mode:',err)}
  }
  window.enterApp=function(){oldEnterApp.apply(this,arguments);setTimeout(detectPartner,80)};
  ...
  async function renderPartnerDashboard(content){
    styles();content.innerHTML='<div class="card"><span class="spinner"></span>Memuat dashboard...</div>';
    ...
    content.innerHTML=`<div class="page-title">Dashboard Guru Partner</div><div class="page-sub">Absensi kelas partner dan nilai Tahfizh sesuai halaqah Anda.</div>
      ...
      <div class="gp-kpis"><div class="gp-kpi"><div class="i">${icon('home')}</div><strong>${st.partner_classes||0}</strong><span>Kelas Partner</span></div><div class="gp-kpi"><div class="i">${icon('calendar')}</div><strong>${att}</strong><span>Absensi Hari Ini</span></div><div class="gp-kpi"><div class="i">${icon('book')}</div><strong>${st.pts_total||0}</strong><span>Siswa Halaqah</span></div><div class="gp-kpi"><div class="i">${icon('chart')}</div><strong>${pts}%</strong><span>Kelengkapan Nilai Tahfizh</span></div></div>
      <div class="gp-actions"><button class="gp-action" onclick="setActiveModule('absensi')"><div class="gp-action-head">${icon('calendar')} Absensi</div><small>Isi Morning Talk dan kehadiran kelas partner. Data yang sudah diisi Walas langsung terbaca.</small></button><button class="gp-action" onclick="setActiveModule('partner-pts')"><div class="gp-action-head">${icon('book')} Nilai Tahfizh</div><small>Klik untuk langsung membuka siswa yang termasuk halaqah Anda. Tidak menampilkan seluruh siswa satu kelas.</small></button></div>`;
  }
  ...
  async function renderPartnerPTS(content){
    styles();dirty=false;rows=[];reports=new Map();content.innerHTML='<div class="card"><span class="spinner"></span>Memuat halaqah Tahfizh...</div>';
    try{boot=await req('bootstrap')}catch(e){content.innerHTML=`<div class="empty-state">${escapeHtml(e.message)}</div>`;return}
    const classes=boot.classes||[];if(!classes.length){content.innerHTML='<div class="empty-state">Belum ada halaqah Tahfizh yang ditugaskan.</div>';return}
    activeClassId=classes[0].id;const c=classes[0];const picker=classes.length>1?`<div class="gp-field"><label>Halaqah</label><select id="gp-class" onchange="gpChangeClass(this.value)">${classes.map(x=>`<option value="${escapeHtml(x.id)}">${escapeHtml(x.name)} · ${escapeHtml(x.halaqah_name||boot.teacher_name||'')}</option>`).join('')}</select></div>`:`<div class="gp-field"><label>Halaqah</label><div class="gp-badge">${icon('book')} ${escapeHtml(c.name)} · ${escapeHtml(c.halaqah_name||boot.teacher_name||'')}</div></div>`;
    content.innerHTML=`<div class="page-title">Nilai Tahfizh</div><div class="page-sub">Daftar siswa otomatis mengikuti halaqah pada akun Guru Partner. Nama siswa memakai data resmi CQlass.</div><div class="card"><div class="gp-toolbar">${picker}<div class="gp-field"><label>Guru Pengisi</label><div class="gp-badge">${icon('user')} ${escapeHtml(boot.teacher_name||currentUser.nama)}</div></div><span class="gp-state" id="gp-state">Tersimpan</span><button class="btn gp-save" id="gp-save" onclick="gpSave()">${icon('save')} Simpan</button></div><div id="gp-grid"></div></div>`;
    await loadClass();
  }

  async function loadClass(){const box=document.getElementById('gp-grid');if(!box)return;box.innerHTML='<div style="padding:18px"><span class="spinner"></span>Memuat siswa halaqah...</div>';try{const d=await req('roster',{class_id:activeClassId});rows=d.students||[];reports=new Map((d.reports||[]).map(r=>[r.student_id,r]));dirty=false;renderGrid(d)}catch(e){box.innerHTML=`<div class="empty-state">${escapeHtml(e.message)}</div>`}}
  function renderGrid(meta={}){const box=document.getElementById('gp-grid');if(!box)return;if(!rows.length){box.innerHTML='<div class="empty-state">Belum ada siswa yang terpetakan pada halaqah ini.</div>';return}box.innerHTML=`<div class="gp-note">Halaqah: <b>${escapeHtml(meta.halaqah_name||boot?.teacher_name||'')}</b> · ${rows.length} siswa. Sistem hanya menampilkan siswa yang sudah dicocokkan dengan kelas dan nama resmi CQlass.</div><div class="gp-tablewrap"><table class="gp-table"><thead><tr><th>No</th><th>Nama Siswa</th><th>Materi Hafalan</th><th>LP Tahfizh</th><th>Realisasi Saat Ini</th><th>Prestasi Tahfizh</th><th>Jml Surat</th><th>Jml Baris</th><th>Jml Ayat</th><th>Jml Baris LP</th><th>Persentase</th><th>Kenaikan Juz</th></tr></thead><tbody>${rows.map((s,i)=>{const r=reports.get(s.id)||{};return `<tr><td style="padding:8px;text-align:center">${i+1}</td><td class="name">${escapeHtml(s.full_name||s.name||'')}</td><td>${cell(s.id,'materi_hafalan',val(r,'materi_hafalan'),true)}</td><td>${cell(s.id,'lp_tahfizh',val(r,'lp_tahfizh'),true)}</td><td>${cell(s.id,'realisasi_saat_ini',val(r,'realisasi_saat_ini'),true)}</td><td>${cell(s.id,'prestasi_tahfizh',val(r,'prestasi_tahfizh'),true)}</td><td>${cell(s.id,'jumlah_surat',val(r,'jumlah_surat'))}</td><td>${cell(s.id,'jumlah_baris',val(r,'jumlah_baris'))}</td><td>${cell(s.id,'jumlah_ayat',val(r,'jumlah_ayat'))}</td><td>${cell(s.id,'jumlah_baris_lp',val(r,'jumlah_baris_lp'))}</td><td>${cell(s.id,'persentase',val(r,'persentase'))}</td><td>${cell(s.id,'mengikuti_kenaikan_juz',val(r,'mengikuti_kenaikan_juz'))}</td></tr>`}).join('')}</tbody></table></div>`;updateState()}

  window.gpDirty=function(){dirty=true;updateState()};function updateState(){const st=document.getElementById('gp-state');if(st){st.textContent=dirty?'Belum disimpan':'Tersimpan';st.classList.toggle('dirty',dirty)}}
  window.gpChangeClass=async function(id){if(dirty&&!confirm('Ada data yang belum disimpan. Tetap pindah halaqah?')){const el=document.getElementById('gp-class');if(el)el.value=activeClassId;return}activeClassId=id;await loadClass()};
  window.gpGridKey=function(e){const inputs=[...document.querySelectorAll('.gp-cell')],i=inputs.indexOf(e.target);if(i<0)return;const cols=10;let n=i;if(e.key==='Enter'||e.key==='ArrowDown')n=i+cols;else if(e.key==='ArrowUp')n=i-cols;else if(e.key==='ArrowRight'&&e.target.selectionStart===e.target.value.length)n=i+1;else if(e.key==='ArrowLeft'&&e.target.selectionStart===0)n=i-1;else return;if(n>=0&&n<inputs.length){e.preventDefault();inputs[n].focus();inputs[n].select()}};
  window.gpGridPaste=function(e){const text=e.clipboardData?.getData('text/plain')||'';if(!/[\t\n]/.test(text))return;const inputs=[...document.querySelectorAll('.gp-cell')],start=inputs.indexOf(e.target);if(start<0)return;e.preventDefault();const matrix=text.replace(/\r/g,'').split('\n').filter((x,i,a)=>x!==''||i<a.length-1).map(r=>r.split('\t')),cols=10;matrix.forEach((r,ri)=>r.forEach((v,ci)=>{const el=inputs[start+ri*cols+ci];if(el){el.value=v;el.dispatchEvent(new Event('input',{bubbles:true}))}}))};
  window.gpSave=async function(){const btn=document.getElementById('gp-save');if(!btn)return;const inputs=[...document.querySelectorAll('.gp-cell')],map=new Map(rows.map(s=>[s.id,{student_id:s.id}]));inputs.forEach(el=>{const x=map.get(el.dataset.student);if(x)x[el.dataset.key]=el.value.trim()});const old=btn.innerHTML;btn.disabled=true;btn.innerHTML='<span class="spinner"></span>Menyimpan...';try{const d=await req('save',{class_id:activeClassId,items:[...map.values()]});dirty=false;updateState();showToast(`${d.saved||0} data siswa berhasil disimpan.`)}catch(e){showToast(e.message||'Gagal menyimpan nilai Tahfizh.',true)}finally{btn.disabled=false;btn.innerHTML=old}};
